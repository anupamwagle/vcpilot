...
def check_entry_triggers(self):
    ...
    if _is_trading_paused():
        logger.debug("Trading paused — skipping entry check")
        return
    ...
    with get_db() as db:
        ...
        if regime == "BEAR":
            logger.info("Market in BEAR regime — no new entries")
            return
        ...
        open_count = db.query(Position).filter(Position.status == TradeStatus.OPEN).count()
        max_positions = int(engine.threshold("portfolio_max_positions") or 5)

        if open_count >= max_positions:
            logger.debug(f"Max positions reached ({open_count}/{max_positions})")
            return
    ...
    for signal in pending_signals:
        try:
            ...
            if sizing.shares < 1:
                logger.warning(f"Signal {signal.ticker}: position size too small ({sizing.message})")
                continue
            ...
            logger.info(f"Entry submitted: {signal.ticker} {sizing.shares}x @ {entry_price:.3f}")

        except Exception as e:
            logger.error(f"Entry trigger error for {signal.ticker}: {e}")
    ...
    for pos in positions:
        try:
            ...
            for exit_sig in exit_signals:
                ...
                logger.info(f"Exit: {pos.ticker} {exit_sig.reason} P&L ${pnl_aud:+.0f}")
                break  # Process one exit signal at a time

        except Exception as e:
            logger.error(f"Exit check error for {pos.ticker}: {e}")
