...
instead of through the web app:
  ...
  2. Point your reverse proxy so `/mcp/sse` and `/mcp/messages` go to this
     container's port instead of the web app's, while `/mcp/oauth/token`
     and `/authorize` keep going to the web app (they need the web app's
     login session and are not served here).
  3. Remove the `app.mount("/mcp", create_mcp_app())` line from
     web/main.py once traffic is confirmed flowing through here, to
     avoid running the tool-serving logic in two places at once.

Shares the same Postgres DB and APP_SECRET_KEY as the web app (both read
from the same .env), so Bearer JWTs minted by the web app's
