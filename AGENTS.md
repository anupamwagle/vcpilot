...
Any task wired to a dashboard button must surface failure in the Task Log, not just container logs — otherwise the button "does nothing" from the user's perspective. `_run_screen_force`, `evaluate_market_regime_task`, `recategorise_watchlist_labels`, `refresh_asx_sector_data`, and `send_daily_report` (per-org AND loop-level) now write an `AuditLog(action=TASK_ERROR)` row in their outer except blocks (wrapped in its own try/except so audit failure can't mask the original error). `app/tasks/trading.py` already followed this pattern extensively. **Follow it for any new manually-triggerable task.** Also (14 Jul 2026): task summary audit messages must report the FULL breakdown, not a bare success count — "5/754 assigned" reads like a bug when 749 items were simply already correct. `recategorise_watchlist_labels` reports changed/already-correct/unmatched(+sample)/protected; `refresh_price_data` reports stored/no-data/empty/stale(+sample); `refresh_live_prices_cache_task` names the tickers with no live source; `evaluate_market_regime_task` writes TASK_ERROR when the benchmark fetch fails (previously a silent return that left the regime stale); `refresh_asx_sector_data`'s outer except now writes TASK_ERROR too (a crash there previously left the chained re-categorise running on blank sector data with no trace).
...
- [ ] `/superadmin/organizations` → AW → MCP Credentials → Generate (all scopes) → configure in Codex Desktop
